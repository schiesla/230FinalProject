...
public class MapDrawer extends JPanel implements MouseListener {
	...
	private static final double TOP_LEFT_CORNER_LAT = 41.0000;
	private static final double TOP_LEFT_CORNER_LONG = 109.000;
	private static final Point2D.Double TOP_LEFT_CORNER = new Point2D.Double(TOP_LEFT_CORNER_LONG, TOP_LEFT_CORNER_LAT);

	private static final double TOP_RIGHT_CORNER_LAT = 41.0000;
	private static final double TOP_RIGHT_CORNER_LONG = 102.000;
	private static final Point2D.Double TOP_RIGHT_CORNER = new Point2D.Double(TOP_RIGHT_CORNER_LONG,
			TOP_RIGHT_CORNER_LAT);

	private static final double BOTTOM_LEFT_CORNER_LAT = 37.000;
	private static final double BOTTOM_LEFT_CORNER_LONG = 109.0000;
	private static final Point2D.Double BOTTOM_LEFT_CORNER = new Point2D.Double(BOTTOM_LEFT_CORNER_LONG,
			BOTTOM_LEFT_CORNER_LAT);

	private static final double BOTTOM_RIGHT_CORNER_LAT = 37.000;
	private static final double BOTTOM_RIGHT_CORNER_LONG = 102.0000;
	...
	private static double FRAME_MULTIPLIER = 150;
	private static final double MAP_RADIUS = FRAME_MULTIPLIER / 4.2;
	...
	private LinkedList<Map.PointofInterest> path;
...
//				(int) (750 - (100 - this.FRAME_MULTIPLIER - this.zoomCount * 6.2)), this);
		...
		g2.drawRect((int) (-(FRAME_WIDTH / 2.0) + this.panelCenterX - 15 - (this.zoomCount * 3.5)),
				(int) (-(FRAME_HEIGHT / 2.0) + this.panelCenterY - 5 - (this.zoomCount * 2)),
				(int) (FRAME_WIDTH - (100 - this.FRAME_MULTIPLIER - this.zoomCount * 6)),
	...
	public void setRouteButtonPressed(LinkedList<Map.PointofInterest> path) {
		this.path = path;
	...
	public void drawMap(Graphics2D g) throws IOException {
		...
		if (this.routeButtonPressed) {

			for (int i = 0; i < this.path.size() - 1; i++) {

				double lat1 = this.path.get(i).getLatitude();
				double longit1 = this.path.get(i).getLongitude();
				Point2D.Double point1 = new Point2D.Double(longit1 - CENTER_LONG, lat1 - CENTER_LAT);

				double lat2 = this.path.get(i + 1).getLatitude();
				double longit2 = this.path.get(i + 1).getLongitude();
				Point2D.Double point2 = new Point2D.Double(longit2 - CENTER_LONG, lat2 - CENTER_LAT);

				Line2D.Double road = new Line2D.Double(point1.getX() * -FRAME_MULTIPLIER,
						point1.getY() * -FRAME_MULTIPLIER, point2.getX() * -FRAME_MULTIPLIER,
						point2.getY() * -FRAME_MULTIPLIER);
